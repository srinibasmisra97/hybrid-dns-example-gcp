...
resource "google_compute_subnetwork" "app_subnet" {
  name          = "${google_compute_network.app.name}-sbn"
  network       = google_compute_network.app.self_link
  ip_cidr_range = "10.0.10.0/24"
}

resource "google_compute_firewall" "app_vpc_allow_iap" {
  name          = "${google_compute_network.app.name}-allow-iap"
  network       = google_compute_network.app.self_link
...
resource "google_compute_firewall" "app_vpc_allow_dns" {
  name          = "${google_compute_network.app.name}-allow-dns"
  network       = google_compute_network.app.self_link
